...
작성일: 2026-04-05
최종 수정: 2026-04-09 (전체 완료 반영, 인프라 스프린트 I-1~I-4 추가)
상태: Phase 0~2 완료 / 인프라 스프린트 진행 예정
...
## 현재 진행 현황 (2026-04-09 기준)

| 단계 | 기간 | 상태 | 비고 |
|------|------|------|------|
| Phase 0: 환경 준비 + 프로젝트 구조 | 04-07 ~ 04-08 | ✅ 완료 (예정대로) | NodeKit/NodeForge 초기 구조 완성 |
| Phase 1: L1 검증 + WasmPolicyChecker | 04-09 ~ 04-16 계획 → **04-09 완료** | ✅ 완료 (7일 선행) | L1 검증, DockGuard wasm, GrpcPolicyBundleProvider 포함 |
| Phase 2: NodeForge L2~L3 + PolicyService | 04-17 ~ 04-23 계획 → **04-09 완료** | ✅ 완료 (2주 선행) | BuildService, ValidateService, CatalogService, 62개 단위 테스트, multipass-k8s-lab 통합 테스트 |
| NodeKit Bug Fix Sprint | 04-08 ~ 04-09 | ✅ 완료 | 21개 이슈 해결, Phase 2 UI 완성 |
| **Sprint I-1: Flannel → Cilium 마이그레이션** | 예정 | ⏳ 대기 | multipass-k8s-lab CNI 교체 |
| **Sprint I-2: Harbor Helm + HTTPRoute** | 예정 | ⏳ 대기 | I-1 완료 후 진행 |
| **Sprint I-3: buildah/podbridge5 전환** | 예정 | ⏳ 대기 | kaniko → buildah, I-2와 병렬 가능 |
| **Sprint I-4: NodeForge K8s Deployment** | 예정 | ⏳ 대기 | Cilium GRPCRoute 연동, I-2/I-3 완료 후 |

**핵심 아키텍처 결정 (스프린트 중 확정)**:
- builder workload: **buildah/podbridge5** (kaniko 대체) — NodeForge는 관리자 권한 호스트 바이너리이므로 K8s Pod 보안 제약 무관
- 레지스트리: **Harbor** (Helm 설치 예정) — TLS, 취약점 스캔, robot account
- CNI: **Cilium** (Flannel에서 마이그레이션 예정) — Gateway API + artifact-handoff mesh 기반으로 필수
- 테스트 환경: **multipass-k8s-lab** (3노드 VM 클러스터, K8s v1.32.13) — kind와 병행
...
**빌더 구현 기술**:
**buildah/podbridge5** (확정). NodeForge는 관리자 권한으로 실행되는 호스트 바이너리이므로 K8s Pod 보안 제약(CAP_SETUID 등)의 영향을 받지 않는다. podbridge5(`/opt/go/src/github.com/HeaInSeo/podbridge5`)는 buildah Go 라이브러리를 직접 wrapping하며, `BuildConfig.CreateImageWithDockerfile(ctx, store)` API를 통해 subprocess 없이 빌드한다. 당초 kaniko를 사용했으나, buildah와의 코드 재사용성 및 관리자 환경과의 자연스러운 통합으로 전환 결정.
...
### 3.5 테스트 환경 (kind + multipass-k8s-lab)

**kubeconfig 기반 연결**: NodeForge는 kubeconfig로 K8s API에 직접 접근한다. Ingress/서비스메시는 미사용 — Sprint I-4에서 Cilium GRPCRoute로 대체 예정.

| 환경 | kubeconfig | 레지스트리 주소 | 비고 |
|------|-----------|----------------|------|
| kind | `~/.kube/config` | `10.96.0.1:5000` | 단일 노드, 로컬 |
| **multipass-k8s-lab** (권장) | `../multipass-k8s-lab/kubeconfig` | `10.87.127.18:31500` | Ubuntu 24.04 VM 3대, K8s v1.32.13 |

**multipass-k8s-lab 클러스터 구성**:
- master: `lab-master-0` (10.87.127.18), worker: `lab-worker-0` (10.87.127.94), `lab-worker-1` (10.87.127.150)
- CNI: 현재 Flannel → **Sprint I-1에서 Cilium으로 마이그레이션 예정**
- 통합 테스트: `make test-integration-multipass` (NodeForge Makefile)

**Cilium 전환 배경**: artifact-handoff 프로젝트(Cilium service mesh 기반 파일 전송)에서 어차피 Cilium이 필요하므로, Flannel→Cilium 마이그레이션을 통해 Harbor HTTPRoute + NodeForge GRPCRoute + artifact-handoff mesh를 동일 스택에서 운영한다.

### 3.6 레지스트리

**현재 (테스트)**: `registry:2` — K8s NodePort로 노출, emptyDir 스토리지, HTTP only.
- kind: ClusterIP `10.96.0.1:5000`
- multipass-k8s-lab: NodePort `10.87.127.18:31500`

**목표 (Sprint I-2)**: **Harbor** (Helm 설치, Cilium HTTPRoute 연동)
- TLS, 취약점 스캔, robot account 지원
- K8s Pod이 `imagePullSecret`으로 pull
- `NODEFORGE_REGISTRY_ADDR` 환경변수로 주소 분리 — 레지스트리 교체 시 코드 변경 없음
...
**NodeKit/NodeForge 스프린트**: 2026-04-07(월) ~ **04-09 조기 완료** (계획 대비 2주 선행)
**인프라 스프린트 I-1~I-4**: 2026-04-09 이후 순차 진행

```
Week 1  04-07 ~ 04-08   [완료] Phase 0: 환경 준비 + 프로젝트 구조
        04-09            [완료] Phase 1: L1 검증 + WasmPolicyChecker (계획: 6일 → 실제: 1일)
                         [완료] Phase 2: NodeForge L2~L3 + PolicyService (계획: 04-17~23 → 실제: 04-09)
                         [완료] NodeKit Bug Fix Sprint

Sprint I-1  예정         Flannel → Cilium CNI 마이그레이션 (multipass-k8s-lab)
Sprint I-2  I-1 후       Harbor Helm 설치 + Cilium HTTPRoute + harbor-suspend/resume 스크립트
Sprint I-3  I-2와 병렬   NodeForge kaniko → buildah/podbridge5 전환
Sprint I-4  I-2/I-3 후   NodeForge K8s Deployment + Cilium GRPCRoute

별도 트랙   진행 중       artifact-handoff: Cilium mesh 기반 파일 전송 (연구/문서화)
```

---

### ✅ 완료된 작업 요약 (2026-04-09)

| 항목 | 결과 |
|------|------|
| NodeKit Phase 0~2 | 완료 (계획 대비 2주 선행) |
| NodeKit Bug Fix Sprint | 완료 (21개 이슈 해결) |
| DockGuard security/genomics 규칙 추가 | 완료 (DSF001-003, DGF001-002) |
| NodeForge gRPC 서버 (5개 서비스) | 완료 |
| NodeForge 단위 테스트 62개 | 완료 (PASS) |
| multipass-k8s-lab 통합 테스트 환경 | 완료 (3노드 VM) |
| NodeForge 통합 테스트 (happy/bad path) | 완료 (PASS) |
| NodeForge README (한국어) | 완료 |
| 아키텍처 결정: buildah/podbridge5, Harbor, Cilium | 완료 (확정) |

---

### Infrastructure Sprint I-1: Flannel → Cilium CNI 마이그레이션
**대상**: multipass-k8s-lab (lab-master-0, lab-worker-0, lab-worker-1)

| 작업 | 완료 기준 |
|------|-----------|
| Flannel 제거 | Flannel DaemonSet 삭제, CNI 파일 정리 |
| Cilium Helm 설치 | `cilium status` 모든 컴포넌트 OK |
| 노드 상태 확인 | `kubectl get nodes` — 전체 Ready |
| Pod 간 통신 확인 | 기존 workload 정상 동작 |
| Gateway API CRD 설치 확인 | GatewayClass 등록 확인 |

**리스크**:
- CNI 교체 중 노드 간 네트워크 단절 → 순차 작업, 백업 kubeconfig 준비
- Cilium 버전과 K8s v1.32 호환성 확인 필요

---

### Infrastructure Sprint I-2: Harbor Helm + Cilium HTTPRoute
**선행 조건**: I-1 완료

| 작업 | 완료 기준 |
|------|-----------|
| Harbor Helm 설치 | Harbor UI 접근 가능 |
| TLS 인증서 설정 | HTTPS 접근 |
| Cilium HTTPRoute 설정 | 외부 → Harbor 라우팅 |
| harbor-suspend/harbor-resume 스크립트 | Harbor Pod 일시 중지/재개 자동화 |
| NodeForge 레지스트리 주소 전환 | `NODEFORGE_REGISTRY_ADDR` 업데이트 |
| K8s imagePullSecret 등록 | smoke run Pod에서 Harbor pull 성공 |

---

### Infrastructure Sprint I-3: buildah/podbridge5 전환
**선행 조건**: I-2와 병렬 가능

| 작업 | 완료 기준 |
|------|-----------|
| podbridge5 Go 라이브러리 NodeForge 연동 | `BuildConfig.CreateImageWithDockerfile` 호출 성공 |
| kaniko Job spec 제거 | ConfigMap + kaniko Job 생성 코드 삭제 |
| buildah 빌드 → Harbor push | digest 확보 성공 |
| 기존 통합 테스트 통과 | `make test-integration-multipass` PASS |
| 단위 테스트 업데이트 | BuildService 단위 테스트 buildah mock으로 교체 |

---

### Infrastructure Sprint I-4: NodeForge K8s Deployment + Cilium GRPCRoute
**선행 조건**: I-2, I-3 완료

| 작업 | 완료 기준 |
|------|-----------|
| NodeForge Deployment YAML 작성 | `kubectl apply` 성공 |
| Cilium GRPCRoute 설정 | 외부 gRPC → NodeForge 라우팅 |
| NodeKit gRPC 엔드포인트 전환 | localhost:50051 → GRPCRoute 주소 |
| 전체 흐름 end-to-end 테스트 | NodeKit → NodeForge (K8s) → Harbor 전체 동작 |
| in-cluster ServiceAccount 인증 전환 | kubeconfig 직접 접근 → ServiceAccount |

---

### Phase 0 — 환경 준비 + 프로젝트 구조
**기간**: 2026-04-07(월) ~ 2026-04-08(화) / 2일 ✅ **완료**
...
**기간**: 계획 2026-04-09(수) ~ 04-16(수) → **실제 04-09 완료** ✅
...
**기간**: 계획 2026-04-17(목) ~ 04-23(수) → **실제 04-09 완료** ✅
...
**기간**: 계획 2026-04-24(목) ~ 04-29(화) → **실제 04-09 완료** ✅ (Phase 2 통합 완료 시점에 함께 완료)
...
### 확정 계획 (Infrastructure Sprint)

| 항목 | 스프린트 | 상태 |
|------|---------|------|
| Flannel → Cilium CNI 마이그레이션 | I-1 | ⏳ 대기 |
| Harbor Helm + Cilium HTTPRoute | I-2 | ⏳ I-1 후 |
| buildah/podbridge5 전환 (kaniko 제거) | I-3 | ⏳ I-2와 병렬 |
| NodeForge K8s Deployment + Cilium GRPCRoute | I-4 | ⏳ I-2/I-3 후 |

### 로드맵 항목

| 항목 | 설명 | 우선순위 |
|------|------|---------|
| DockGuard K8s Admission Webhook | K8s Job 제출 시점에도 정책 검사 | 중 |
| 정책 서버 + 자동 갱신 | OPA Bundle Server, 폴링 기반 자동 업데이트 | 낮 |
| 지속형 builder service | Job-per-build → Deployment / StatefulSet 기반 (빌드 캐시 지속, 병렬 빌드 지원) | 낮 |
| 이름 기반 I/O 매핑 검증 | "출력 수 == 입력 수" 규칙을 이름 기반으로 교체 | DagEdit 연동 시 |
| ToolDefinition 버전 관리 | CAS 기반에서 semantic versioning 또는 content hash 전략 추가 | 낮 |

### 별도 실험 트랙

| 항목 | 설명 | 상태 |
|------|------|------|
| artifact-handoff | Cilium service mesh 기반 파일 전송. multipass-k8s-lab에서 실험 중. | 연구/문서화 진행 중 |
